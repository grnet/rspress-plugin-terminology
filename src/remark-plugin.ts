...
import { RemarkPluginOptions, TermMetadata } from './types';
...
function findTermInIndex(
  ...
  let normalizedUrl = url.replace(/\.(md|mdx|html)$/, '');
...
export function terminologyRemarkPlugin(
  ...
  return function transformer(this: Processor, tree: Root) {
    ...
    console.log('[remark-plugin] Processing markdown, termIndex size:', termIndex.size);
    ...
    visit(tree, 'link', (node: any) => {
      ...
      const isTermLink = /\.(md|mdx)$/.test(url) || /(^\.\/|^\/)?terms\/[^/]+$/.test(url);
      ...
      console.log('[remark-plugin] Transforming link to Term component:', url, '->', routePath);
